...
import { MembershipCoin } from "../typechain/MembershipCoin";
import { FunStableCoin } from "../typechain/FunStableCoin";

describe("StableCoin", function () {
  let ethUsdPrice: number, feeRatePercentage: number;
  let StableCoin: FunStableCoin;
  ...
  this.beforeEach(async () => {
    ...
    const StableCoinFactory = await ethers.getContractFactory("FunStableCoin");
  ...
  describe("With minted tokens", function () {
    ...
    it("Should prevent depositing collateral buffer below minimum", async function () {
      ...
      ).to.be.revertedWithCustomError(StableCoin,'InitialCollateralRatioError'
      );
    });
    ...
    it("Should allow depositing collateral buffer", async function () {
      ...
      const MembershipCoinFactory = await ethers.getContractFactory(
        "MembershipCoin"
      );
      const MembershipCoin = await MembershipCoinFactory.attach(
        await StableCoin.membershipCoin()
      );

      const newInitialSurplusInUsd = stableCoinCollateralBuffer * ethUsdPrice;
      expect(await MembershipCoin.totalSupply()).to.equal(
    ...
    describe("With deposited collateral buffer", function () {
      ...
      let MembershipCoin: MembershipCoin;
      ...
      this.beforeEach(async () => {
        ...
        MembershipCoin = await MembershipCoinFactory.attach(
          await StableCoin.membershipCoin()
        );
      });

      it("Should allow withdrawing collateral buffer", async function () {
        const newMembershipTotalSupply =
          stableCoinCollateralBuffer * ethUsdPrice;
        const stableCoinCollateralBurnAmount = newMembershipTotalSupply * 0.2;
        ...
        expect(await MembershipCoin.totalSupply()).to.equal(
          ethers.utils.parseEther(
            (
                newMembershipTotalSupply - stableCoinCollateralBurnAmount
            ).toString()
          )
        );
      });
